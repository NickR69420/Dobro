(function (root, factory){
    ...
    var binary2DMapToTiledFn = function(tileSize, fn){
        return function(map){
            ...
            var index;
            var slice;
            var prto = (new Array(tileSize)).fill(0);
            for(var h = 0; h < height; h+= tileSize){
                index = h / tileSize;
                if(!results[index]) results[index] = [];
                for(var w = 0; w < width; w+= tileSize){
                    //console.log('TO ', typeof map[h+3])
                    slice = prto.map(function(val, offset){
                        return map[h+offset]?map[h+offset].slice(w, w+tileSize):[]
                    });
                    results[index][w/tileSize] = fn(slice);
                }
            }
            return results;
        }
    ...
    var blocksFor = function(code){
        if(blocksMap[code]) return blocksMap[code];
        console.log('CODE NOT FOUND:'+code);
        return ' ';
    }
    var miniQuadToBlocks = function(mini){
        ...
        return blocksFor(boolArr2DToIndexes(bits).join(''))
    };
    var quadToBraille = function(quad){
    ...
    var braille = {
        'from' : brailleFor,
        'blocks' : blocksFor,
        miniQuadToBlocks : miniQuadToBlocks,
        quadToBraille : quadToBraille,
        binary2DMapToBraille : binary2DMapToTiledFn(4, quadToBraille),
        binary2DMapToBlocks : binary2DMapToTiledFn(2, miniQuadToBlocks),
