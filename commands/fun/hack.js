  ...
  run: async (bot, message, args) => {
    try {
      ...
      message.channel.send(`Hacking ${user.user.tag}...`).then((msg) => {
        ...
        const mcemoji = bot.emojis.cache.find(
          (emoji) => emoji.name === "mcgif"
        );
        const gf = bot.emojis.cache.find((emoji) => emoji.name === "gurl");
        const fortniteemoji = bot.emojis.cache.find(
          (emoji) => emoji.name === "fortnitedance"
        );
        ...
        let pFiveMessages = [
          `Deleting System32.....`,
          `Uninstalling Anime_Hoties folder...`,
          `Injecting SeggzTrojan #6696`,
          `Corrupting C: Drive...`,
        ];
        ...
        setInterval(() => {
          switch (phaseno) {
            case 1:
              pOneMessages =
                pOneMessages[Math.floor(Math.random * pOneMessages.length)];
              if (pOneMessages == "0") {
                msg.edit(
                  `NEVER MIND! IT BACKFIRED AND YOU'RE GETTING HACKED!!!!`
                );
            ...
            case 2:
              pTwoMessages =
                pTwoMessages[Math.floor(Math.random() * pTwoMessages.length)];
            ...
            case 3:
              pThreeMessages =
                pThreeMessages[
                  Math.floor(Math.random() * pThreeMessages.length)
                ];
            ...
            case 4:
              pFourMessages =
                pFourMessages[Math.floor(Math.random * pFourMessages.length)];
            ...
            case 6:
              pFiveMessages =
                pFiveMessages[Math.floor(Math.random() * pFiveMessages.length)];
